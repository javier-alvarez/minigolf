...
let rec MainLoop () =
  template <- "template" + Math.GetRandomNumber(7).ToString()
...
and HandleKey () =
  ...
  if GraphicsWindow.LastKey = "Up" then
    ...
    let xposbk = xpos
    let mutable yposdelta = 0
    while yposdelta = 0 && abs(xposbk - xpos) < 3 do // move up to 3 times only
...
and CopyPiece () = // in basetemplate, template, rotation 
  ...
  if rotation = "CW" then
    for i = 0 to BOXES - 1 do // x' = y y' = L - 1 - x
      ...
      // new x and y
      let x = v % 10
      let y = L - 1 - int (floor(float v/10.0))
...
and CheckStop () = // in: ypos, xpos, h ret: done
  ...
  if ``done`` = 1 then
    for i = 0 to BOXES - 1 do
      ...
      //x = Math.Floor(v/10)
      //y = Math.Remainder(v, 10)
      let x = int(floor(float v / 10.0)) + xpos
      let y = (v % 10) + ypos - 1
      if y >= 0 then
...
and DeleteLines () =
  ...
  if linesCleared > 0 then
    score <- score + 100 * int (round(float linesCleared * 2.15 - 1.0))
    PrintScore()
...
and PrintScore () =
  ...
  let zeros = "00000000".Substring(0, 8 - scoreText.Length)
  GraphicsWindow.DrawText(485, 70, zeros + scoreText)
