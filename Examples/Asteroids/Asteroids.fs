...
and Move  () =
   // Move player
   let x = (Shapes.GetLeft(player) + (cos(radians(playerAngle - 90.0)) * playerSpeed) + gameWidth) % gameWidth
   let y = (Shapes.GetTop(player) + (sin(radians(playerAngle - 90.0)) * playerSpeed) + gameHeight) % gameHeight
   Shapes.Move(player, x, y)

   // Move rocks
   for i = 0 to rockCount-1 do
     let x = (Shapes.GetLeft(rock.[i]) + (cos(radians(rockAngle.[i] - 90.0)) * rockSpeed) + gameWidth) % gameWidth
     let y = (Shapes.GetTop(rock.[i]) + (sin(radians(rockAngle.[i] - 90.0)) * rockSpeed) + gameHeight) % gameHeight
     Shapes.Move(rock.[i], x, y)

   // Move ammo
   for i = 0 to ammoCount-1 do
     let x = (Shapes.GetLeft(ammo.[i]) + (cos(radians(ammoAngle.[i] - 90.0)) * ammoSpeed) + gameWidth) % gameWidth
     let y = (Shapes.GetTop(ammo.[i]) + (sin(radians(ammoAngle.[i] - 90.0)) * ammoSpeed) + gameHeight)% gameHeight
...
and CollisionCheck () =
   ...
   px1 <- Shapes.GetLeft(player) - ( (abs(playerWidth * cos(radians(playerAngle)) + playerHeight * sin(radians(playerAngle))) - playerWidth) / 2.0)
   py1 <- Shapes.GetTop(player) - ( (abs(playerWidth * sin(radians(playerAngle)) + playerHeight * cos(radians(playerAngle))) - playerHeight) / 2.0)
   px2 <- px1 + abs(playerWidth * cos(radians(playerAngle)) + playerHeight * sin(radians(playerAngle)))
   py2 <- py1 + abs(playerWidth * sin(radians(playerAngle)) + playerHeight * cos(radians(playerAngle)))
